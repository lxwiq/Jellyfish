<script lang="ts">
  ...
  let libraryName = $state<string>('Séries');
  ...
  async function init() {
    ...
    try {
      ...
      if (pid) {
        ...
        const lib = await resolveLibraryId(baseUrl, token, userId, 'series');
        libraryId = lib?.id ?? null;
        libraryName = lib?.name ?? 'Séries';
      }
      pageIndex = 0;
      items = [];
      await loadPage(true);
      ...
      console.error('Failed to init series page', e);
    } finally {
      loading = false;
    }
  }
...
<SafeArea class="mx-auto w-full min-h-[100svh] px-5 sm:px-8 py-6 sm:py-8 space-y-6">
  ...
  {:else}
    <!-- Items -->
    {#if items.length === 0}
      <p class="text-sm text-muted-foreground">Aucune série[تا？re</p>
    {:else}
      {#if viewMode === 'grid'}
        <div class="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3 sm:gap-4">
          {#each items as it (it.Id)}
            <a href={`/series/${it.Id}`} aria-label={it.Name}>
