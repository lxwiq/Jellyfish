<script lang="ts">
  ...
  let { data } = $props<{ data: { item: ItemDetail | null; nextUp?: any | null; seasons?: any[] } }>();
...
{:else}
  ...
  <main class="mx-auto max-w-4xl px-3 sm:px-4 py-4 space-y-6">
    ...
    {#if seasons?.length}
      <section>
        <h3 class="text-base font-semibold mb-3">Saisons</h3>
        <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3 sm:gap-4">
          {#each seasons as s}
            <a href={`/series/${item!.Id}/s/${s.Id}`} class="block group">
              <div class="overflow-hidden rounded-md">
                <div class="relative w-full aspect-[2/3]">
                  <img
                    src={itemPosterUrl(s, { width: 300, height: 450 })}
                    srcset={posterSrcset(s, [200,300,450])}
                    sizes={posterSizes}
                    alt={s.Name}
                    class="absolute inset-0 w-full h-full object-cover group-hover:scale-[1.03] transition-transform"
                    loading="lazy" decoding="async" fetchpriority="low"
                    onerror={(e) => ((e.currentTarget as HTMLImageElement).src = placeholderPosterUrl(300, 450, s.Name))}
                  />
                </div>
              </div>
              <div class="mt-2 text-xs sm:text-sm font-medium line-clamp-2">
                {#if s.IndexNumber != null}S{s.IndexNumber} — {/if}{s.Name}
              </div>
            </a>
          {/each}
        </div>
