...
import { listSeriesPaged, type SortOrder } from "$lib/core/api";
...
export const load: PageLoad = async ({ fetch, url }) => {
  try {
    ...
    const { items, total } = await listSeriesPaged({ page, pageSize, sortBy: sort, sortOrder: order, search: q }, fetch);
